...
class CIPIC_HRIR(object):
    def __init__(self, dirname):
        ...
        for i, phi in enumerate(azimuth):
            ...
            self.right_hrir[:, i, :] = np.flip(librosa.load(os.path.join(dirname, right_fn), sr=None, mono=False)[0], axis=0)
            self.left_hrir[:, i, :] = np.flip(librosa.load(os.path.join(dirname, left_fn), sr=None, mono=False)[0], axis=0)
...
def test_cipic_hrir():
    ...
    hrir = CIPIC_HRIR('/home/xuxudong/git_repo/spatialaudiogen/pyutils/CIPIC_hrtf_database/standard_hrir_database/subject_003')
    res = hrir.get_closest(Position(7*pi/4, -pi/4, 3, 'polar'))
    print(res.shape)
