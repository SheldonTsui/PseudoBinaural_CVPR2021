...
def main():
    ...
    for audio_file in tqdm(audio_list):
        ...
        if 'asmr' in opt.hdf5FolderPath:
            folder_id = video_path.split('/')[-1]
            video_frame_path = video_path.replace(folder_id, 'folder_'+folder_id)
        else:
	...
	#perform spatialization over the whole spectrogram in a siliding-window fashion
        ...
        while sliding_window_start + samples_per_window < audio.shape[-1]:
            ...
            normalizer, audio_segment_mix = audio_normalize(audio_segment_mix)
            audio_segment_channel1 = audio_segment_channel1 / normalizer 
            audio_segment_channel2 = audio_segment_channel2 / normalizer 
            ...
            if 'asmr' in opt.hdf5FolderPath:
                image = Image.open(os.path.join(video_frame_path, '{:06d}.jpg'.format(frame_index))).convert('RGB')
            else:
	...
	#check output directory
        ...
        if librosa.__version__ >= '0.8.0':
            ...
            sf.write(os.path.join(cur_output_dir_root, 'predicted_binaural.wav'), predicted_binaural_audio.transpose(), opt.audio_sampling_rate)
            sf.write(os.path.join(cur_output_dir_root, 'mixed_mono.wav'), mixed_mono.transpose(), opt.audio_sampling_rate)
            sf.write(os.path.join(cur_output_dir_root, 'input_binaural.wav'), audio.transpose(), opt.audio_sampling_rate)
        else:
