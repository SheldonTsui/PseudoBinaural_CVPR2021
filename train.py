...
def compute_loss(output, audio_mix, loss_criterion, prefix="stereo", weight=1, fusion_loss_only=False):
    loss_dict = {}
    
    spec_weight = torch.ones_like(output['audio_gt'])

    if not fusion_loss_only:
        key = '{}_loss'.format(prefix)
        loss_dict[key] = loss_criterion(output['binaural_spectrogram'], output['audio_gt'].detach(), spec_weight) * weight

    if 'pred_left' in output:
        fusion_loss1 = loss_criterion(2*output['pred_left']-audio_mix[:,:,:-1,:], output['audio_gt'].detach(), spec_weight)
        fusion_loss2 = loss_criterion(audio_mix[:,:,:-1,:]-2*output['pred_right'], output['audio_gt'].detach(), spec_weight)
        ...
        for i, val_data in enumerate(data_loader_val):
            if i < opt.validation_batches:
                ...
                if 'stereo' in opt.dataset_mode:
                    ...
                    val_total_loss.update(compute_loss(output, audio_mix, loss_criterion, prefix='stereo', weight=opt.stereo_loss_weight, fusion_loss_only=opt.fusion_loss_only))
                """
                if 'sep' in opt.dataset_mode:
                    frame_sep = val_data['frame_sep'].to(opt.device)
                    sep_diff = val_data['sep_diff_spec'].to(opt.device)
                    sep_mix = val_data['sep_mix_spec'].to(opt.device)
                    img_feat = net_visual(frame_sep)
                    if net_fusion is not None: 
                    ...
                    val_total_loss.update(compute_loss(output_sep, sep_mix, loss_criterion, prefix='sep', weight=opt.sep_loss_weight, fusion_loss_only=opt.fusion_loss_only))
                """
...
if opt.validation_on:
    ...
    opt.mode = 'val'
    dataset_mode_backup = opt.dataset_mode
    opt.dataset_mode = 'stereo'
    data_loader_val = CreateDataLoader(opt)
    opt.mode = 'train' #set it back
    opt.dataset_mode = dataset_mode_backup
...
if opt.loss_mode == 'mse':
    loss_criterion = criterion.L2Loss()
...
for epoch in range(1, opt.niter+1):
    ...
    for i, data in enumerate(data_loader):
        ...
        if 'stereo' in opt.dataset_mode:
            ...
            total_loss.update(compute_loss(output, audio_mix, loss_criterion, prefix='stereo', weight=opt.stereo_loss_weight, fusion_loss_only=opt.fusion_loss_only))

        if 'sep' in opt.dataset_mode:
            frame_sep = data['frame_sep'].to(opt.device)
            ...
            total_loss.update(compute_loss(output_sep, sep_mix, loss_criterion, prefix='sep', weight=opt.sep_loss_weight, fusion_loss_only=opt.fusion_loss_only))
